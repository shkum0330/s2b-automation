...
public class PromptBuilder {
    ...
    private static final String PRODUCT_SPEC_PROMPT_TEMPLATE = """
        ...
        [허용 출처]
        1) 제조사 공식 한국어 웹사이트 2) 다나와(danawa.com)
        → 위 2개 이외의 출처는 절대 사용 금지.
    ...
    private static final String CERTIFICATION_PROMPT_TEMPLATE = """
        [역할]
        당신은 한국 제품의 KC인증번호를 검증하는 정밀 데이터 분석가입니다.

        [목표]
        주어진 모델명과 일치하거나 해당 모델명을 포함하는 'KC 전파적합성인증번호', '국가기술표준원 전기용품 인증번호'를 찾아 지정된 JSON 형식으로 출력합니다.

        [핵심 규칙]
        1. **모델명 매칭 규칙**: 인증 문서 내에, 주어진 모델명('%s')과 **정확히 일치하는 모델명** 또는 **해당 모델명을 일부로 포함하는 파생/상세 모델명**이 명확하게 명시되어야 합니다.
        2. **신뢰 출처**: '제품안전정보센터의 KC인증정보 검색서비스(KC 전파적합성인증번호)', '국립전파연구원(국가기술표준원 인증번호)' 또는 '제조사 공식 웹사이트', '다나와(danawa.com)'만 신뢰합니다.
        3. 결과가 여러 개일 경우 하나만 가져와야 합니다. 단 우선순위는 맨 끝자리가 알파벳이 붙은 겁니다.
        4. **결과 처리**: 찾지 못하거나, 모델명 매칭이 불확실하면 반드시 빈 문자열("")로 처리해야 합니다.
