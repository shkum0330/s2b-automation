...
public class PromptBuilder {
    ...
    private static final String CERTIFICATION_PROMPT_TEMPLATE = """
        ...
        [목표]
        주어진 모델명과 일치하거나 해당 모델명을 포함하는 '국가기술표준원 전기안전인증번호'와 'KC 전파적합성인증번호'를 찾아 지정된 JSON 형식으로 출력합니다.

        [핵심 규칙]
        1. **모델명 매칭 규칙**: 인증 문서 내에, 주어진 모델명('%s')과 **정확히 일치하는 모델명** 또는 **해당 모델명을 일부로 포함하는 파생/상세 모델명**이 명확하게 명시되어야 합니다.
        2. **신뢰 출처**: 'KC인증정보 검색서비스' 또는 '제조사 공식 웹사이트'만 신뢰합니다.
        3. **결과 처리**: 찾지 못하거나, 모델명 매칭이 불확실하면 반드시 빈 문자열("")로 처리해야 합니다.

        [출력 형식]
        다른 설명 없이, 아래 JSON 형식만 반환하세요.
        ```json
        {"katsCertificationNumber":"(찾아낸 전기안전인증번호)","kcCertificationNumber":"(찾아낸 KC 전파적합성인증번호)"}
        ```""";
